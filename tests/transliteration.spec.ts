...
// 1. Data-Driven Testing: Unga Excel scenarios-ai array-aaga maatriyullaen
const testCases = [
  ...
  { id: 'Pos_Fun_0005', input: 'Enakku oru email anuppunga, Zoom link mela irukku.', expected: 'எனக்கு ஒரு email அனுப்புங்க, Zoom link மேல இருக்கு.' },
  ...
  { id: 'Neg_Fun_0007', input: 'Long text input test scenario', expected: 'அவுட்புட் தாமதமாகலாம்.' },
...
// 2. Loop moolam ovvoru test-aiyum automate seithen
for (const data of testCases) {
  test(`Test Case ${data.id}: Transliterating "${data.input}"`, async ({ page }) => {
    // 3. Website-ukku chellungal
    await page.goto('https://tamil.changathi.com/');
    
    // 4. Input box-il data-vai fill seiyungal
    await page.locator('#TLTextArea').fill(data.input);
    
    // 5. Processing-ukku oru chinna delay (1.5 seconds)
    await page.waitForTimeout(1500); 
    
    // 6. Actual output-ai edungal
    const actualOutput = await page.locator('#TLTextArea').inputValue();
    
    console.log(`Input: ${data.input} | Expected: ${data.expected} | Actual: ${actualOutput}`);
    
    // 7. Optional: Match aaguthaa endru check seiya (Failures-ai report-il kaatta)
